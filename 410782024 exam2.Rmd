...
tpe_highSchools<-factor(c(tpe_highSchools,"臺北市立第一女中","台北市立第一女子高級中學","中山女中","中山女高", "台北市立中山女中", "臺北市立中山女子高級中學","國立政大附中", "國立政治大學附屬高級中學"))
tpe_highSchools
...
```{r ans13}
# regex_hs1st
regex_hs1st<-"臺?台?北?市?立?第?一?女?高?級?中?學"
...
data228有四個元素：
  ...
  * 附註：前述個人名稱的出處文件
	...
	A504000000F/0037/牘簿160/1/1/018	臺灣高等法院	江東碧	表冊名稱:臺灣高等法院受理內亂案件簡表  
### 2.1 部門編號/年份
...
pick_103<-str_detect(courseStructure$`學年`,"103")
courseStructure$`應休系級`[pick_103]
pick_econ101junior<-str_detect(courseStructure$`應修系級`[pick_103],"經濟系3")
courseStructure$`科目名稱`[pick_econ101junior]->econ101junior
...
pick_"入學年" <- example$name == "小新"
map(entranceYear,~.x[pick_"入學年"])->a
map(major,~.x[pick_"系別"])->b
coursesOffered<-list(a,b)
