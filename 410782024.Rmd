...
```{r ans11}
violatedArticles<-unique(factor(badEmployers$law))
violatedArticles
...
```{r ans13}
# mostViolatedArticle
pickabout500 <-(table_countsByArticle>500)
mostViolatedArticle <- (table_countsByArticle[pickabout500])
mostViolatedArticle
...
mostViolatedArticle_content<-max(levels(factor(badEmployers$lawContent)))
...
print(students10$`姓名`[ntpu_namelist])
```
...
```{r ans23}
# ntpu_econtpu_eco
econ1 <- c(a<-students10$`國文`>=12,b<-c(students10$`英文`,students10$`數學`)>=26)
econ2 <-  stringr::str_detect(students10$`志願`,"北大經濟")
ntpu_eco<- students10$`姓名`[econ1&econ2]
ntpu_econtpu_eco<- students10$`姓名`[econ1&econ2]
ntpu_econtpu_eco
