...
logi<-TRUE
approve<-logi
course<-list(coursename="會計學",type="必",credits=3)
...
```{r ans32}
# courseUpdate
courseUpdate<-list(coursename="會計學",type="必",credits=3)
time_slots<-list("每星期三78節","每星期二5節")
time_place<-list(time_slots,classroom="商2F05")
courseUpdate
```
...
```{r ans33}
# accidents
tpeTime<-ymd_hms(2014-01-01,tz="Asia/Taipei")
bureaus<-c("板橋分局","海山分局")
`板橋分局`<-list(type="駕駛人過失-其他",casualties=1)
...
levels<-c(5,3,2,1,4)
storage_fiveReceipts<-as.integer(levels)
storage_fiveReceipts
```
...
# chr_fiveReceipts
levels<-c(5,3,2,1,4)
chr_fiveReceipts<-as.character(levels)
chr_fiveReceipts
